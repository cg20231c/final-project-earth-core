...
to get that animation and character we can use asset from https://www.mixamo.com/#/?page=1&type=Motion%2CMotionPack and dont forget set like this setting if you want to download it (for skin you can go with skin or not):
![image](https://github.com/cg20231c/final-project-earth-core/assets/116022017/3621b368-24fd-4313-8c3e-724e9088490f)
...
For the map, we use a city generator to create the buildings, roads, lamps, and all the environmental elements. This city generator also provide the traffic simulation including the vehicles and the traffic lamps. Initially we have prepared some city design and then did some experiments on creating the environment. During our work, we kept improving our search to get the best environmental setting for the game. We took our search on every source possible including Unity Asset Store itself. We have tried several city design including the New York City Model by Geopipe. For such a semi-open-world game, we finally decided to use a city generator since it provides a massive city scape and the details within seconds. However, the city generator could not be just taken for granted due to some limitation from the program itself, such the colliders of the buildings which not entirely shaped exactly and precisely the same as the building. For the level design, we created an objective for the player to collect all the coins scattered over the map. There is a counter to track the number of collected coins on the upper left side of the screen, so player will not lose track on their findings. These coins are placed with a particular planning. The coins are located around the player at the very beginning of the game, then when the player has collected the surrounding coins, the player needs to platforming and grappling from one building to another to get the next coins. The coins are put in different levels of building height, so the player can harness every platforming and grappling abilities gradually during the search of the coins. If there is a coin nearby, there must be another one around but on a different level of height (building height).
